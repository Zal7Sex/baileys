...
// ── Zal7Sex Custom Logger ──────────────────────────────────────────────────
const chalk = require("chalk");
const _tc = (str) => str.replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.slice(1));
const zLog = {
  info    : (msg) => console.log(`${chalk.bold.blue('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.blue(']')}\n${chalk.bold.blue('└──$')} ${chalk.blueBright(_tc(msg))}\n`),
  success : (msg) => console.log(`${chalk.bold.magenta('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.magenta(']')}\n${chalk.bold.magenta('└──$')} ${chalk.magentaBright(_tc(msg))}\n`),
  warn    : (msg) => console.log(`${chalk.bold.red('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.red(']')}\n${chalk.bold.red('└──$')} ${chalk.redBright(_tc(msg))}\n`),
  error   : (msg) => console.log(`${chalk.bold.red('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.red(']')}\n${chalk.bold.red('└──$')} ${chalk.redBright(_tc(msg))}\n`),
  conn    : (msg) => console.log(`${chalk.bold.blue('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.blue(']')}\n${chalk.bold.blue('└──$')} ${chalk.blueBright(_tc(msg))}\n`),
  qr      : (msg) => console.log(`${chalk.bold.magenta('┌─[')} ${chalk.bold.white('Zal7Sex')} ${chalk.bold.magenta(']')}\n${chalk.bold.magenta('└──$')} ${chalk.magentaBright(_tc(msg))}\n`),
};
// ──────────────────────────────────────────────────────────────────────────
/**
...
const makeSocket = (config) => {
    ...
    const validateConnection = async () => {
        ...
        zLog.conn(`Handshake sent → WhatsApp Web server`);
        ...
        if (!creds.me) {
            ...
            zLog.warn(`Not logged in · Attempting registration...`);
            logger.info({ node }, 'not logged in, attempting registration...');
        }
        else {
            node = (0, Utils_1.generateLoginNode)(creds.me.id, config);
            zLog.conn(`Credentials found · Logging in...`);
    ...
    const uploadPreKeys = async (count = Defaults_1.INITIAL_PREKEY_COUNT) => {
        await keys.transaction(async () => {
                zLog.info(`Uploading pre-keys  →  ${count} keys`);
        logger.info({ count }, 'uploading pre-keys');
            const { update, node } = await (0, Utils_1.getNextPreKeysNode)({ creds, keys }, count);
            await query(node);
            ev.emit('creds.update', update);
                zLog.success(`Pre-keys uploaded successfully`);
        logger.info({ count }, 'uploaded pre-keys');
        });
    };
    const uploadPreKeysToServerIfRequired = async () => {
        const preKeyCount = await getAvailablePreKeysOnServer();
        zLog.info(`Pre-keys on server: ${preKeyCount}`);
    ...
    const end = (error) => {
        ...
        closed = true;
        if (error) {
            zLog.error(`Connection errored · ${error.message || 'Unknown error'}`);
        } else {
            zLog.warn(`Connection closed · Socket terminated`);
        }
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        const genPairQR = () => {
            ...
            zLog.qr(`Scan QR code in WhatsApp · expires in ${Math.round(qrMs / 1000)}s`);
    ...
    ws.on('CB:iq,,pair-success', async (stanza) => {
        ...
        try {
            ...
            zLog.success(`Pairing success · ${updatedCreds.me?.name || updatedCreds.me?.id} (${updatedCreds.platform || 'unknown platform'}) · Restarting...`);
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            zLog.success(`Connection opened · WhatsApp Web connected ✓`);
    ...
    ws.on('CB:stream:error', (node) => {
        zLog.error(`Stream error · ${node?.tag || 'unknown'}`);
    ...
    process.nextTick(() => {
        ...
        zLog.conn(`Connecting to WhatsApp Web...`);
    ...
    ws.on('CB:ib,,offline', (node) => {
        ...
        zLog.info(`Offline sync complete · ${offlineNotifs} pending notification(s) processed`);
